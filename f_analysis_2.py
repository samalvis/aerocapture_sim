...
def gen_real_atm(start_num, stop_num, filename, body_obj, craft_obj, lin_sol_arr):
    ...
    for idx, name in enumerate(names):  # Note: could also do multiple reps on each atmosphere, accounting for other variations introduced?
        ...
        solution = eject_calc(a_f_targ, craft.x_0, body, craft)
        save_output(solution, f"Sim_Data/real_sol_{filename}")
        # plot_case(solution)
        a = 0
        a_dot = 0
        ...
        save_output(f_dist[idx], f"Sim_Data/real_f_{filename}")
...
def plot_f_dist(f_dist, n_bins):  # n_bins from number of
    ...
    plt.xlabel("Scale factor used")
    plt.ylabel("Frequency")
    plt.title("Distribution of linear scale factors used under varied conditions")
    plt.show()
...
if __name__ == "__main__":
    ...
    # SC.x_0[4] = EFPA_calc(a_f_targ, SC.x_0, Mars, SC).craft_obj.x_0[4]
    # print(math.degrees(SC.x_0[4]))
    SC.x_0[4] = math.radians(-9.875)  # old: ~-9.5?  old old: -9.875

    # load linear distributions
    # lin_sols = load_lin_atm("test1_fix")

    # OR generate distributions
    lin_sols = gen_lin_atm(0.05, 0.8, 1.2, "test_full", a_f_targ, Mars, SC)

    # load atmospheres
    # f_dist = load_real_atm("test1_fix")

    # OR generate atmospheres (remember to disperse!)
    f_dist = gen_real_atm(1, 10, "test_full", Mars, SC, lin_sols)  # return solutions? which?
